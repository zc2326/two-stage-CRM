...
#####  Matching scenarios with SGE_TASK_ID
...
para.mat = para[which(para[, 4] >= (para[, 3] * 7)), ]
...
for (ii in 1:length(theta.candi)){
  if(ii==currind) {
    t <- two_stg_crm( para.mat[ii,1],  7, para.mat[ii,2], para.mat[ii,4], para.mat[ii,3])
  }
}


write.csv(t, file = paste0("seven", currind, ".csv"))
